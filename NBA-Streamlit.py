...
st.write("In this app we are going to take a look at how 3 point shots evolved from 1980. ")
st.write("Here is my LinkedIn profile ----> [Ege Can Koç](https://www.linkedin.com/in/egeckoc/)")
...
st.write("Also, since 3 point shot was made official in 1980, we are only going to look at the data starting from that year. We will also change the year column to date format.")
...
""")
st.markdown('##')
st.write("Now we will look at the amount of attempted shots over years.")
...
plt.legend()
st.pyplot(graph1)
st.write("We can see that the amount of 3 point shots are increasing over the years. We can clearly say that NBA teams are preferring 3 point shots more and more and they will keep doing so in the future "
...
st.write("Now we are going to look at general averages data over the years. With this data we will try to see if the 3 point percentage is increasing or decreasing over the years")
...
plt.legend()
st.pyplot(graph2)
st.write("Here we can see that the percentage of 3 point shot has been increasing steadily leaguewide until aroun 2000. After that it seems to stay around the same value with yearly fluctuations")
st.code("""
graph2_seasons = np.arange(1980,2022)
graph2_3pp = general_averages['3P%'].iloc[::-1]
graph2_fgg = general_averages['FG%'].iloc[::-1]
...
st.write("We can now look at the current season and see the 3 point percentages of the teams with the context of the historical data")
...
st.write("Miami Heat has the best percentage in the league with 37.9% and Oklahoma City Thunder has the worst percentage with 32.3%. The league average is 35.4%")
st.markdown("##")
